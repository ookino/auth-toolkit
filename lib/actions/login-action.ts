...
import { sendTwoFactorTokenMail, sendVerificationEmail } from '../mail';
...
export async function login(values: z.infer<typeof LoginSchema>) {
  const validatedEntries = LoginSchema.safeParse(values);

  if (!validatedEntries.success) {
    return { error: 'Invalid entries' };
  }

  const { email, password, code } = validatedEntries.data;
  ...
  try {
    ...
    if (error instanceof AuthError) {
      switch (error.type) {
        ...
        default:
          return { error: 'Something went wrong111111' };
      }
    }
